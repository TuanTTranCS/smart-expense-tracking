...
Phase 1 integration work remains in progress. The main open items are validating on-device LiteRT-LM receipt extraction and implementing and verifying the Windows Hermes/PowerShell Excel update flow. Its workbook mapping, duplicate-handling rules, and safe rollout design are now defined; the remaining work is the processor, automated tests, copied-workbook canary, and paused Hermes job. See [the current status](docs/current%20status.md), [the implementation plan](docs/PLAN.md), and [the requirements](docs/requirements.md) for details.
